...
@section('title', 'Services Suite')
@section('page_title', 'Manage Clinical Services & Category Tabs')
    ...
    <table>
      <thead>
        <tr>
          ...
          <th style="min-width: 250px;">Features (One per line)</th>
          <th style="min-width: 130px;">Button Text</th>
          <th style="min-width: 160px;">Button Link (href)</th>
          <th style="min-width: 140px;">Care Model</th>
            ...
            <form action="{{ route('admin.services.update', $srv) }}" method="POST">
              ...
              <td>
                <select name="category" class="form-control">
                  <option value="primary" {{ $srv->category === 'primary' ? 'selected' : '' }}>Primary</option>
                  <option value="home" {{ $srv->category === 'home' ? 'selected' : '' }}>Home</option>
                  <option value="telehealth" {{ $srv->category === 'telehealth' ? 'selected' : '' }}>Telehealth</option>
                  <option value="certs" {{ $srv->category === 'certs' ? 'selected' : '' }}>Certs</option>
                </select>
              </td>
              ...
              <td>
                <div style="display: flex; gap: 0.5rem; align-items: center; white-space: nowrap;">
                  ...
                  <form action="{{ route('admin.services.destroy', $srv) }}" method="POST" onsubmit="return confirm('Delete service?')">
                    @csrf
                    @method('DELETE')
    ...
    <div class="grid-2">
      <div class="form-group">
        ...
        <select name="category" class="form-control">
          ...
          <option value="certs">Medical Certificates</option>
        </select>
      </div>
      ...
      <div class="form-group">
        <label class="form-label">Care Model</label>
